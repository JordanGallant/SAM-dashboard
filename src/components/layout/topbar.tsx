...
export function Topbar({ showSidebarTrigger = true }: { showSidebarTrigger?: boolean }) {
  ...
  return (
    <header className="flex h-14 items-center gap-3 border-b border-[#0F3D2E]/10 bg-white/90 backdrop-blur px-4 sticky top-0 z-30">
      ...
      <Link
        href="/deals"
        className="flex items-center gap-2 rounded-full border border-[#0F3D2E]/10 bg-white px-3 py-1 hover:border-[#0F3D2E]/25 hover:bg-[#FAFAF7] transition-colors"
      ...
      >
        <div className="flex h-5 w-5 items-center justify-center rounded-md bg-gradient-to-br from-[#0F3D2E] to-[#00A86B]">
          <Building2 className="h-3 w-3 text-[#B5D33C]" />
        </div>
        <span className="text-[11px] font-mono font-semibold uppercase tracking-widest text-[#0F3D2E]">
          {fund?.name ?? "Your Fund"}
        </span>
      </Link>
      ...
      <div className="ml-auto flex items-center gap-1">
        <Link
          href="/settings"
          className="flex h-8 w-8 items-center justify-center rounded-full text-[#0F3D2E]/70 hover:bg-[#FAFAF7] hover:text-[#0F3D2E] transition-colors"
          aria-label="Settings"
        >
