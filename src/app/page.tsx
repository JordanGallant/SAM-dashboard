// SAM landing — production homepage.
...
function Pricing() {
  ...
  const tiers: Tier[] = [
    ...
    },
    {
      ...
      // Fund tier is sales-only — route the CTA straight to the Calendly
      // booking page instead of /register (Fund signups can't self-serve).
      ctaHref: "https://calendly.com/samvc",
      highlighted: false,
    },
  ]
  return (
...
function FAQ() {
  ...
  const items = [
    ...
    },
    {
      ...
      a: "Missing information is flagged separately — it does not automatically reduce the score. A missing CAC figure is not the same as a bad CAC figure. Sam identifies the gap and turns it into a founder follow-up question, so you know exactly what to ask before spending more time on the deal.",
    },
    {
      q: "Does a low score mean we should pass?",
      a: "No. A low score is not an automatic pass recommendation. It means Sam found weaknesses, missing evidence, or risk factors in the available materials. You may still take the call if you have relevant context, founder knowledge, or a thesis-driven reason to explore. Sam frames the discussion — it does not make the decision.",
    },
    {
      q: "Can Sam replace our analyst or associate?",
      a: "No. Sam supports analysts and associates — it does not replace them. It handles repetitive first-screening work (structure, risks, consistency checks, founder questions). Your team still owns judgment, diligence, founder conversations, and conviction. Sam helps analysts spend less time turning messy decks into structured notes and more time on the deals that deserve deeper work.",
    },
    {
      q: "Why not build this internally with ChatGPT?",
      a: "You can build a simple AI summary workflow internally. The harder part is making it consistent, fund-specific, source-aware, and usable across a team. Sam combines a fixed six-domain framework, stage-aware scoring, source attribution, missing-information logic, the Ask Sam co-pilot, fund-fit scoring, and exportable decision records. That is the difference between a prompt and a workflow.",
    },
    {
      q: "Is Sam useful for angels and family offices, or only VC funds?",
      a: "Sam is built for any investor reviewing early-stage dealflow, regardless of fund size. Angels use Sam as a structured second opinion before a first call. Family offices and syndicates use it for a shared first screen. VC funds use it to make first-pass assessments more consistent across analysts, associates, and partners.",
    },
  ]
